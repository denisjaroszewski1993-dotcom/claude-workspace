# Werbeschild MobileSauna-Hamburg (75 × 95 cm)

- `Werbeschild_75x95cm_Druckdatei.pdf` – Druckdatei 1:1, 756 × 956 mm (inkl. 3 mm Beschnitt rundum)
- `Werbeschild_Vorschau.png` – Bildschirm-Vorschau
- `foto-sauna-zugeschnitten.jpg` – verwendeter Fotoausschnitt (Gemini-Wasserzeichen abgeschnitten)

Montage: Unterkante 110 cm, Oberkante 205 cm über Boden.

| Element | Schrift | Versalhöhe | Lesbar bis ca. |
|---|---|---|---|
| SAUNA | Barlow Condensed ExtraBold, weiß | 16 cm | 50–60 m |
| BAU & VERMIETUNG | Barlow Condensed SemiBold, orange | 6 cm | 20 m |
| EINFAHRT → (Band in Orange) | Barlow Condensed ExtraBold, dunkel | 6,4 cm, Pfeil 12 × 7,4 cm | 20–25 m |
| 🌐 www.mobilesauna-hamburg.de | Barlow Condensed SemiBold, weiß/orange, Globus-Icon Ø 5 cm | 3,9 cm | 12–15 m |

Farben: Hintergrund #1C1916 (fast schwarz), Akzent #F2A23A (Sauna-Orange), Schrift #FFFFFF.

Hinweis: Das Foto liegt nur mit 512 px Breite vor (≈ 17 dpi auf 75 cm) – für den Druck
das Originalbild in hoher Auflösung (mind. ca. 3000 px Breite) einsetzen.
